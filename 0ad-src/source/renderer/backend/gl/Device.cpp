...
#if OS_SWITCH
	// De-risk probe for VAO caching: did the GL 4.3 VAO / vertex_attrib_binding entry
	// points load from this Mesa context? (Loaded by name in lib/ogl.cpp.)
	LOGMESSAGE("SWITCHVAO available=%d", ogl_switchVAOAvailable ? 1 : 0);
#endif
...
// glGetProgramivARB is GL_ARB_vertex/fragment_program (legacy assembly shaders),
// which has no core equivalent and is absent on a core Mesa context -> guard it.
#define VERTEXPROGRAM(id) do { \
	GLint i = -1; \
	if (!glad_glGetProgramivARB) break; \
