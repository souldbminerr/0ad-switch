...
#if OS_SWITCH
// Draw the real 0 A.D. cursor sprite (named by the game via SetCursor, e.g.
// "action-gather-fruit" when hovering berries) at the mouse position, since the
// Switch has no visible OS/SDL hardware cursor. Single-entry cache: the cursor
// name changes rarely and only one is shown at a time, so we reload on change.
// The underlying texture is cached by the texture manager regardless.
static void SwitchDrawCursor(CCanvas2D& canvas, const float x, const float y)
{
	const CStrW name = g_VideoMode.GetCursorName();
	if (name.empty())
		return;

	static CStrW s_Name;
	static CTexturePtr s_Texture;
	static VfsPath s_Png;
	static int s_HotspotX = 0;
	static int s_HotspotY = 0;
	...
	if (name != s_Name)
	{
		...
		s_HotspotY = 0;
		s_Texture.reset();
		const VfsPath base(VfsPath(L"art/textures/cursors") / name);

		// Hotspot: the pixel of the sprite drawn at the mouse position.
		std::shared_ptr<u8> buffer;
		size_t size;
		if (g_VFS->LoadFile(base.ChangeExtension(L".txt"), buffer, size) == INFO::OK)
		{
			std::wstringstream s(std::wstring(reinterpret_cast<const wchar_t*>(buffer.get()), size));
			s >> s_HotspotX >> s_HotspotY;
		}

		// Decode and upload the cursor PNG ourselves. We can't go through the
		// texture manager: it converts non-cached textures with NVTT (to build the
		// DDS cache), which isn't compiled into the Switch build, so cursor sprites
		// (never cached -- desktop draws them via SDL) fail with "NVTT not available".
		// Tex::decode + a direct backend upload is exactly what the old SDL cursor
		// path did, minus NVTT.
		s_Png = base.ChangeExtension(L".png");
		std::shared_ptr<u8> file;
		size_t fileSize;
		Tex t;
		if (g_VFS->LoadFile(s_Png, file, fileSize) == INFO::OK &&
			t.decode(file, fileSize) == INFO::OK)
		{
			// Tightly-packed RGBA: drop compression/mipmaps, force RGBA byte order + alpha.
			size_t flags = t.m_Flags;
			flags &= ~static_cast<size_t>(TEX_DXT);
			flags &= ~static_cast<size_t>(TEX_MIPMAPS);
			flags &= ~static_cast<size_t>(TEX_BGR);
			flags |= TEX_ALPHA;
			if (t.transform_to(flags) == INFO::OK)
			{
				namespace B = Renderer::Backend;
				const uint32_t cw = static_cast<uint32_t>(t.m_Width);
				const uint32_t ch = static_cast<uint32_t>(t.m_Height);
				const size_t dataSize = static_cast<size_t>(cw) * ch * (t.m_Bpp / 8);
				B::Sampler::Desc sampler = B::Sampler::MakeDefaultSampler(
					B::Sampler::Filter::LINEAR, B::Sampler::AddressMode::CLAMP_TO_EDGE);
				std::unique_ptr<B::ITexture> backendTexture =
					g_VideoMode.GetBackendDevice()->CreateTexture2D("switch-cursor",
						B::ITexture::Usage::TRANSFER_DST | B::ITexture::Usage::SAMPLED,
						B::Format::R8G8B8A8_UNORM, cw, ch, sampler);
				if (backendTexture)
				{
					g_Renderer.GetDeviceCommandContext()->UploadTexture(
						backendTexture.get(), B::Format::R8G8B8A8_UNORM, t.get_data(), dataSize);
					s_Texture = g_Renderer.GetTextureManager().WrapBackendTexture(
						std::move(backendTexture));
				}
			}
		}
		if (!s_Texture)
			LOGERROR("SWITCHCURSOR failed to load %s", s_Png.string8().c_str());
	}

	if (!s_Texture)
		return;

	const float w = static_cast<float>(s_Texture->GetWidth());
	const float h = static_cast<float>(s_Texture->GetHeight());
	const float dx = x - static_cast<float>(s_HotspotX);
	const float dy = y - static_cast<float>(s_HotspotY);
	canvas.DrawTexture(s_Texture, CRect(dx, dy, dx + w, dy + h));

	static int s_Log = 0;
	if ((s_Log++ % 120) == 0)
		LOGMESSAGE("SWITCHCURSOR png=%s wxh=%.0fx%.0f hs=%d,%d at=%.1f,%.1f",
			s_Png.string8().c_str(), w, h, s_HotspotX, s_HotspotY, dx, dy);
}
#endif
...
void CRenderer::RenderFrame2D(const bool renderGUI, const bool renderLogger)
{
	...
	if (renderGUI)
	{
		...
		// objects correctly. The GUI manager creates its own per-page canvas (each
		// page may use a different scale), so we pass the device context rather
		// than the shared canvas.
		g_GUI->Draw(m->deviceCommandContext.get());

#if OS_SWITCH
		// Switch software cursor: draw the real 0 A.D. cursor sprite (contextual,
		// driven by the game via SetCursor) right after the GUI.
		{
			const float scale = g_VideoMode.GetScale();
			SwitchDrawCursor(canvas,
				static_cast<float>(g_mouse_x) / scale,
				static_cast<float>(g_mouse_y) / scale);
		}
#endif
	}
