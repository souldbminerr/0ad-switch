...
bool Mod::EnableMods(const std::vector<CStr>& mods, const bool addPublic)
{
	...
	{
		std::string en, inc;
		for (const CStr& m : m_EnabledMods) { en += m.c_str(); en += " "; }
		for (const CStr& m : m_IncompatibleMods) { inc += m.c_str(); inc += " "; }
		LOGMESSAGE("SWITCHDBG EnableMods: addPublic=%d enabled=[ %s] incompatible=[ %s]", (int)addPublic, en.c_str(), inc.c_str());
	}
...
void Mod::UpdateAvailableMods(const ScriptInterface& scriptInterface)
{
	...
	LOGMESSAGE("SWITCHDBG UpdateAvailableMods: modPath='%s' found %zu dirs", modPath.string8().c_str(), modDirs.size());
	for (const OsPath& d : modDirs)
		LOGMESSAGE("SWITCHDBG   modDir: '%s'", d.string8().c_str());
	...
	for (DirectoryNames::iterator iter = modDirs.begin(); iter != modDirs.end(); ++iter)
	{
		...
		{
			LOGMESSAGE("SWITCHDBG   ParseModJSON FAILED for '%s'", iter->string8().c_str());
			continue;
		}
		LOGMESSAGE("SWITCHDBG   available mod: pathname='%s' name='%s' ver='%s'", data.m_Pathname.c_str(), data.m_Name.c_str(), data.m_Version.c_str());
