...
#if OS_SWITCH
# include <switch.h>
// Switch loading is CPU-bound (VFS decompression, JS sim setup). Raise the CPU
// boost clock while the loader is active and drop back to normal when idle so we
// don't keep the higher clock (and fan/heat) during gameplay.
static bool s_switchLoadBoosted = false;
static void SwitchSetLoadBoost(bool on)
{
	if (on == s_switchLoadBoosted)
		return;
	appletSetCpuBoostMode(on ? ApmCpuBoostMode_FastLoad : ApmCpuBoostMode_Normal);
	s_switchLoadBoosted = on;
}
#else
static void SwitchSetLoadBoost(bool) {}
#endif
