...
#if OS_SWITCH
	// The Switch has no usable OS/SDL hardware cursor, so the renderer draws the
	// cursor sprite itself. The game still drives the cursor contextually via
	// SetCursor() (e.g. "action-gather-fruit" when hovering berries); this returns
	// the current cursor name so the renderer can draw the matching sprite.
	CStrW GetCursorName() const;
#endif
