...
Paths::Paths(const CmdLineArgs& args)
{
	...
	else // OS-specific path handling
	{
		...
		// Everything lives on the SD card under /switch/0ad. Game data (the
		// 'mods' tree, incl. public + mod) is read from m_rdata = Root()/data,
		// i.e. /switch/0ad/data/mods/. No game-vs-user data distinction.
		const OsPath appdata = OsPath("/switch/0ad");
		m_gameData = appdata/"data"/"";
		m_userData = m_gameData;
		m_cache = appdata/"cache"/"";
		m_config = appdata/"config"/"";
...
#elif OS_SWITCH
	return OsPath("/switch/0ad"); // SD card; data + mods under /switch/0ad/data
#else
