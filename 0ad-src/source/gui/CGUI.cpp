...
InReaction CGUI::HandleEvent(const SDL_Event_* ev)
{
	...
	else if (ev->ev.type == SDL_MOUSEMOTION)
	{
		...
		m_MousePos = CVector2D((float)ev->ev.motion.x / m_Scale, (float)ev->ev.motion.y / m_Scale);
	...
	{
		m_MousePos = CVector2D((float)ev->ev.button.x / m_Scale, (float)ev->ev.button.y / m_Scale);
	}
...
{
	return CSize2D{static_cast<float>(g_xres) / m_Scale, static_cast<float>(g_yres) / m_Scale };
}
