...
#if OS_SWITCH
// Implemented in build/switch/src/switch_keyboard.c: pops the Switch software
// keyboard seeded with `initial`, writes the edited UTF-8 result to `out`,
// returns 1 on accept and 0 on cancel.
extern "C" int switchShowKeyboard(const char* initial, const char* guide, char* out, unsigned long outSize);
#endif
...
#if OS_SWITCH
		// Clicking an editable field opens the Switch software keyboard, seeded
		// with the field's current text so it can be edited (and re-edited on a
		// later click). The result is written back as if typed.
		if (!m_Readonly)
		{
			char out[2048];
			const std::string initial = utf8_from_wstring(*m_Caption);
			if (switchShowKeyboard(initial.c_str(), "", out, sizeof(out)))
			{
				std::wstring text = wstring_from_utf8(out);
				if (m_MaxLength != 0 && text.length() > static_cast<size_t>(m_MaxLength))
					text.resize(static_cast<size_t>(m_MaxLength));
				m_Caption.GetMutable().assign(text);
				m_iBufferPos = static_cast<int>(text.length());
				m_iBufferPos_Tail = -1;
				UpdateText();
				UpdateBufferPositionSetting();
				UpdateAutoScroll();
...
#if OS_SWITCH
		// No hardware keyboard on the Switch. We open the on-screen keyboard on an
		// explicit click of the field (GUIM_MOUSE_PRESS_LEFT) rather than on focus,
		// so fields that auto-focus when a screen loads don't pop the keyboard with
		// nothing to type into. Don't call SDL_StartTextInput here either.
		break;
#else
