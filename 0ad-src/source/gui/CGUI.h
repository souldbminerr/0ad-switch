...
public:
	...
	/**
	 * GUI scale for this page. Normally the global gui.scale, but the page
	 * manager gives popup/overlay pages a different (smaller) scale so they stay
	 * a readable size on screen while the main UI is scaled up. Affects layout
	 * (GetWindowSize), input coordinate mapping, and the draw canvas.
	 */
...
private:
	...
	// GUI scale for this page (see GetScale). Initialised to the global scale.
	float m_Scale = 1.0f;
