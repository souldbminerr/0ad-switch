...
public:
	...
	/**
	 * See CGUI::Draw; applies to @em all loaded pages. Each page is drawn on its
	 * own canvas at its own scale (the bottom/main page uses the global gui.scale;
	 * popup/overlay pages above it use a smaller popup scale), so the constructed
	 * canvas is created here per page rather than passed in.
	 */
	void Draw(Renderer::Backend::IDeviceCommandContext* deviceCommandContext) const;
