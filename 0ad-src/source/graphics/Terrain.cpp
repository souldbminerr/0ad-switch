...
{
	// May be called by the camera while a map is still loading (heightmap not yet
	// allocated); treat an unloaded terrain as flat ground.
	if (!m_Heightmap || m_MapSize < 2)
		return 0.0f;
...
{
	// The camera (CCameraController::FocusHeight) can query ground level while a
	// map (e.g. the main-menu background) is still loading and the heightmap isn't
	// allocated yet; treat that as flat ground rather than dereferencing null.
	if (!m_Heightmap || m_MapSize < 2)
		return 0.0f;
