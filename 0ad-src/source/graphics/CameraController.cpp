...
#if OS_SWITCH
	// The cursor is driven by the left stick, so mouse-edge scrolling fights with
	// it constantly (the pointer sits at a screen edge and pans forever). Disable
	// it; the right stick pans the camera instead (see SwitchUpdateControllerMouse).
	m_ViewScrollMouseDetectDistance = 0;
#endif
